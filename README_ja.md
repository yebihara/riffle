...
Riffleは、Railsのページネーションに「DBトランザクションのスナップショット分離」相当の
一貫性をアプリ層で持ち込むgemです。
初回リクエスト時に検索結果のIDリストをRedisに凍結することで、
**ページ移動間の Phantom Read を排除**します。
ユーザーは何度ページを行き来しても、レコードの追加・削除・並び替えに惑わされず、
同じスナップショットを見続けられます。
...
3. 初回リクエスト時にそのページに表示されるはずだったレコードと同一の結果を返す

キャッシュの TTL がスナップショットの寿命です。
TTL内であればユーザーは凍結された再現可能なビューを見続けられます。
長期トランザクションがクエリのスナップショットを保持し続けているのと同等の体験です。
